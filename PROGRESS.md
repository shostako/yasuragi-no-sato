...
### Phase 7: メール通知機能（2026-01-16）
- [x] Resendパッケージ導入
- [x] /api/notify API Route作成
- [x] 予約フォーム送信時にメール通知
- [x] 問い合わせフォーム送信時にメール通知
- [x] Vercel環境変数設定（RESEND_API_KEY, ADMIN_EMAIL）
- [x] 本番動作確認

### Phase 8: Codexレビュー対応（2026-01-16）
- [x] ダブルブッキング競合状態修正（Firestoreトランザクション化）
- [x] 予約ステータス更新のアトミック化（バッチ処理）
- [x] /api/notify バリデーション強化・HTMLエスケープ追加
...
- [x] 新規予約・問い合わせの通知機能（Resendメール通知）✅ 2026-01-16完了
...
| 優先度 | 問題 | 詳細 | 状態 |
|--------|------|------|------|
| 高 | 無認証書き込み | contacts/reservationsの`allow create: if true`、フィールド検証なし | **対応済**（Security Rules） |
| 中 | newsクエリ不整合 | news取得で`where(published == true)`がなかった | **対応済** |
| 中 | XSSリスク | news詳細ページでサニタイズなし | **対応不要**（管理者=オーナーのため） |
| 低 | admin/setup矛盾 | role更新がrulesで禁止されている | **対応済**（環境変数ガード） |
