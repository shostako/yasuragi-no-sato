...
## 2026-01-17 インライン編集機能（Phase 1）実装

### 実装内容
- **EditableText コンポーネント**: contentEditableを使用した編集可能テキスト
- **PageContentContext**: Firestoreのページコンテンツ管理（pageContentsコレクション）
- **firestore.rules**: pageContentsコレクションのread/write権限追加

### 対応ページ・セクション

| ページ | コンポーネント | pageId |
|--------|---------------|--------|
| トップ | HeroSection | home |
| トップ | ServicesSection | home |
| トップ | FeaturesSection | home |
| トップ | CTASection | home |
| 会社概要 | PageHeader | about |
| 会社概要 | AboutCTASection | about |
| 採用情報 | RecruitHeroSection | recruit |
| 採用情報 | RecruitCTASection | recruit |

### 技術的ポイント
- サーバーコンポーネント（page.tsx）から各セクションをクライアントコンポーネントに分離
- 各セクションでPageContentProviderを使用し、pageIdごとにFirestoreからコンテンツを取得
- 編集モード時のみcontentEditable有効化、ホバーで枠線表示
- 楽観的更新 + ロールバックでUX向上

### 未対応
- サービス詳細ページ（/services/[slug]）はデータがマスタから来ているためスキップ
- 画像編集、フォント・色変更はPhase 2以降
